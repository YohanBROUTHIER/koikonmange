...
export default class RecipeValidator extends CoreValidator {
  static checkBodyForCreate({ name, image, steps, hunger, time, preparatingTime, person },{id}) {
    if (!name || !String(name).match(/^[a-zA-Z][a-zA-Z0-9\u00E0-\u00EF\u00F9-\u00FC' .-]{3,}$/)) {
      throw new AppError("Merci de renseigner le nom correctement.", {name: "Bad Request", httpStatus:400});
    }
    if (image && !String(image)) {
      throw new AppError("Erreur de chemin de l'image.", {name: "Bad Request", httpStatus:400});
    }
    if (!steps) {
      throw new AppError("Merci d'indiquer les étapes de la préparation.", {name: "Bad Request", httpStatus:400});
    }
    steps.forEach((step, index) => {
      if (!String(step)) {
        throw new AppError(`Merci de renseigner l'étape ${index+1} correctement.`, {name: "Bad Request", httpStatus:400});
      }
    });
    if (hunger && !["Copieux","Normal","Léger"].includes(hunger)) {
      throw new AppError(`hunger n'accepte que les valeur: Copieux, Normal, Léger`, {name: "Bad Request", httpStatus:400});
    }
